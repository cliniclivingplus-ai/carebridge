...
// username -> display name for everyone who appears in the report (physio, Sales/Doctor).
async function teamNames() {
  try {
    ...
    return (username) => (team.find((t) => t.username === username) || {}).name || username || '';
  } catch {
    return (username) => username || '';
  }
}

// The visit these notes belong to, for the timeline (on the way, arrived, check-in location...).
async function linkedVisit(session) {
  if (!session.visitId) return null;
  try {
    const store = db.isConfigured() ? require('./visits-pg') : require('./visits');
    return await store.getVisit(session.visitId);
  } catch {
    return null;
  }
}
...
async function syncSession(session, parentCase) {
  ...
  try {
    // An uploaded PhysioWay form goes to Clinicea as it is; otherwise CareBridge builds the report.
    const uploaded = session.formFileId ? await sessionFiles.getFile(session.formFileId) : null;
    if (session.formFileId && !uploaded) throw new Error('The uploaded form could not be found');
    const pdf = uploaded
      ? uploaded.data
      : await buildSessionReport(session, parentCase, { visit: await linkedVisit(session), nameOf: await teamNames() });
