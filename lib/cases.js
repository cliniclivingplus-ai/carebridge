...
async function recordSessionFeedback(caseId, { beforeAssessment, afterSummary, clinicalNotes, sessionDate, physioUsername }) {
  ...
  const sync = await syncSession(newSession, targetCase);
  newSession.cliniceaSyncStatus = sync.status;
  newSession.cliniceaSyncError = sync.error;
  newSession.cliniceaEncounterId = sync.encounterId;
  saveSessions(sessionsList);
...
async function retrySessionSync(sessionId) {
  ...
  session.cliniceaEncounterId = sync.encounterId;
