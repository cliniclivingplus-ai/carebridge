...
async function listTeam() {
  const pool = getPool();
  const { rows } = await pool.query('SELECT username, name FROM partners ORDER BY name');
  return rows;
}

module.exports = { findByUsername, verifyLogin, canSeePatient, createPartner, listTeam };
