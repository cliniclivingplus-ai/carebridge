...
async function listTeam() {
  const pool = getPool();
  const { rows } = await pool.query('SELECT username, name, email, phone FROM partners ORDER BY name');
  return rows.map((r) => ({ username: r.username, name: r.name, email: r.email || '', phone: r.phone || '' }));
}

async function createPartner({ username, password, name, allowedPatientIds, email, phone }) {
  const pool = getPool();
  const passwordHash = await bcrypt.hash(password, 10);
  await pool.query(
    `INSERT INTO partners (username, password_hash, name, allowed_patient_ids, email, phone)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash, name = EXCLUDED.name, allowed_patient_ids = EXCLUDED.allowed_patient_ids, email = EXCLUDED.email, phone = EXCLUDED.phone`,
    [username, passwordHash, name, JSON.stringify(allowedPatientIds || []), email || '', phone || '']
  );
}
...
async function registerUser({ username, password, name, email, phone }) {
  ...
  await pool.query(
    `INSERT INTO partners (username, password_hash, name, allowed_patient_ids, email, phone)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [username.trim(), passwordHash, name.trim(), JSON.stringify([]), (email || '').trim(), (phone || '').trim()]
  );
