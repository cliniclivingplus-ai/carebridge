...
// Session reports are attached to the patient's record in Clinicea as PDF documents. Off until
// CLINICEA_SESSION_SYNC=true, so nothing is written to Clinicea until the clinic switches it on.
function isSessionSyncEnabled() {
  return isLiveMode() && process.env.CLINICEA_SESSION_SYNC === 'true';
}

// Attach a PDF to the patient's documents ("Clinical Report"). Verified against the live API:
//   1. documents/createDocumentDetails registers it; Clinicea assigns the document ID and the
//      storage name (<organisation>-<document ID><file name>). docDtm must be a plain date.
//   2. documents/getFolder gives a SAS key for Clinicea's Azure container; the PDF is uploaded
//      there under exactly that storage name.
// If the upload fails, the registration is deleted again so no broken link stays on the patient.
async function attachPatientDocument(cliniceaPatientId, fileName, pdfBuffer, docDate) {
  if (!cliniceaPatientId) throw new Error('Case has no Clinicea internal patient ID');
  const date = /^\d{4}-\d{2}-\d{2}$/.test(docDate || '')
    ? docDate
    : new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });

  const params = new URLSearchParams({
    patientID: cliniceaPatientId,
    docName: fileName,
    docCatg: 'Clinical Report',
    docDtm: date,
    docFKType: '1', // PatientDocument
    docFKID: cliniceaPatientId,
  });
  const doc = await cliniceaFetch(`/api/v3/documents/createDocumentDetails?${params}`, { method: 'POST' });
  if (!doc || !doc.ID || doc.SavingErrorCode || doc.SavingErrorText) {
    throw new Error(`Clinicea did not register the document: ${(doc && (doc.SavingErrorText || doc.SavingErrorCode)) || 'no document ID returned'}`);
  }

  try {
    const folder = await cliniceaFetch('/api/v3/documents/getFolder');
    if (!folder || !folder.AccountSAS || !folder.ContainerName || !folder.BlobURI || !doc.DocumentOnlineName) {
      throw new Error('Clinicea did not return an upload location');
    }
    const sas = folder.AccountSAS.startsWith('?') ? folder.AccountSAS : `?${folder.AccountSAS}`;
    const res = await fetch(`${folder.BlobURI}/${folder.ContainerName}/${encodeURIComponent(doc.DocumentOnlineName)}${sas}`, {
      method: 'PUT',
      headers: { 'x-ms-blob-type': 'BlockBlob', 'Content-Type': 'application/pdf' },
      body: pdfBuffer,
    });
    if (res.status !== 201) throw new Error(`file upload failed (${res.status})`);
  } catch (err) {
    const del = new URLSearchParams({ docID: doc.ID, patientID: cliniceaPatientId });
    await cliniceaFetch(`/api/v3/documents/deleteDocumentByPatient?${del}`, { method: 'DELETE' }).catch(() => {});
    throw new Error(`Clinicea document upload failed: ${err.message}`);
  }
