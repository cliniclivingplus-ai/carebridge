...
async function addPatientEncounter(patientId, encounterNotes) {
  if (!isLiveMode()) {
    return mock.updateNotes(patientId, encounterNotes);
  }
  try {
    const params = new URLSearchParams({ patientID: patientId, notes: encounterNotes });
    return await cliniceaFetch(`/api/v3/patientVisits/createEncounterFull?${params}`, { method: 'POST' });
  } catch (err) {
    // Fallback to updateAppointmentNotes if encounter route is restricted
    return updateAppointmentNotes(patientId, encounterNotes);
  }
}
