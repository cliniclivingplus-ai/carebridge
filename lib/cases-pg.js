...
async function recordSessionFeedback(caseId, { beforeAssessment, afterSummary, clinicalNotes, sessionDate, physioUsername }) {
  const pool = getPool();
  ...
  try {
    ...
    const inserted = await client.query(
      `INSERT INTO sessions (id, case_id, patient_id, session_number, scheduled_date, physio_username, status, before_assessment, after_summary, clinical_notes, clinicea_sync_status)
       VALUES ($1, $2, $3, $4, COALESCE($9::timestamptz, now()), $5, 'completed', $6, $7, $8, 'pending')
       RETURNING *`,
      [sessionId, caseRow.id, caseRow.patient_id, nextSessionNumber, physioUsername, JSON.stringify(beforeAssessment || {}), JSON.stringify(afterSummary || {}), notes, sessionDate || null]
    );
  ...
  // Clinicea is called outside the transaction -- a slow external API must not hold the row lock.
  const session = mapSession(sessionRow);
  const sync = await syncSession(session, mapCase(caseRow));
  await pool.query(
    `UPDATE sessions SET clinicea_sync_status = $2, clinicea_sync_error = $3, clinicea_encounter_id = $4, updated_at = now() WHERE id = $1`,
    [session.id, sync.status, sync.error, sync.encounterId]
  );

  return {
    case: await getCase(caseRow.id),
    session: { ...session, cliniceaSyncStatus: sync.status, cliniceaSyncError: sync.error, cliniceaEncounterId: sync.encounterId },
  };
}
...
async function retrySessionSync(sessionId) {
  ...
  await pool.query(
    `UPDATE sessions SET clinicea_sync_status = $2, clinicea_sync_error = $3, clinicea_encounter_id = $4, updated_at = now() WHERE id = $1`,
    [sessionId, sync.status, sync.error, sync.encounterId]
  );
  return { ...session, cliniceaSyncStatus: sync.status, cliniceaSyncError: sync.error, cliniceaEncounterId: sync.encounterId };
}
