...
async function recordSessionFeedback(caseId, { beforeAssessment, afterSummary, clinicalNotes, sessionDate, physioUsername, visitId, formFileId, formFileName }) {
  const pool = getPool();
  ...
  try {
    ...
    const inserted = await client.query(
      `INSERT INTO sessions (id, case_id, patient_id, session_number, scheduled_date, physio_username, status, before_assessment, after_summary, clinical_notes, clinicea_sync_status, visit_id, form_file_id, form_file_name)
       VALUES ($1, $2, $3, $4, COALESCE($9::timestamptz, now()), $5, 'completed', $6, $7, $8, 'pending', $10, $11, $12)
       RETURNING *`,
      [sessionId, caseRow.id, caseRow.patient_id, nextSessionNumber, physioUsername, JSON.stringify(beforeAssessment || {}), JSON.stringify(afterSummary || {}), notes, sessionDate || null, visitId || null, formFileId || null, formFileName || null]
    );
...
function mapSession(row) {
  ...
  return {
    ...
    formFileId: row.form_file_id || null,
    formFileName: row.form_file_name || null,
    status: row.status,
