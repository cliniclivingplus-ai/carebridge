...
function row(a) {
  return {
    ...
    notifiedNewBooking: a.notified_new_booking,
  };
}
...
async function setAssignedTo(appointmentId, assignedTo) {
  const pool = getPool();
  const { rows } = await pool.query(
    `UPDATE appointments SET assigned_to = $2, updated_at = now() WHERE appointment_id = $1 RETURNING *`,
    [appointmentId, assignedTo || null]
  );
  if (!rows[0]) return null;
  return row(rows[0]);
}
...
async function markNotified(appointmentId) {
  const pool = getPool();
  await pool.query('UPDATE appointments SET notified_new_booking = true WHERE appointment_id = $1', [appointmentId]);
}
