...
function createVisitService(store) {
  ...
  async function markNotesSubmitted(caseId, visitId, sessionId, user) {
    ...
    if (visit.status !== 'completed') {
      visit.statusHistory.push(historyEntry('completed', user, { note: 'Session recorded from the case card; the visit steps (on the way, arrived, started) were not used' }));
    }
