...
function createVisitService(store) {
  ...
  function assertAssignedPhysio(visit, user) {
    if (visit.assignedPhysio !== user.username) {
  ...
  async function stop(visitId, outcome, user, reason) {
    ...
    const visit = await load(visitId);
    if (!isStaff(user)) assertAssignedPhysio(visit, user);
  ...
  async function findVisitForNotes(caseId, visitId, user) {
    ...
    const visit = visitId
      ...
      : visits.find((v) => v.status === 'completed' && v.assignedPhysio === user.username);
    return visit && visit.status === 'completed' ? visit : null;
  }

  // Called after session notes are saved: closes the visit they belong to.
  async function markNotesSubmitted(caseId, visitId, sessionId, user) {
    const visits = await store.listVisitsForCase(caseId);
    const visit = visitId
      ? visits.find((v) => v.id === visitId)
      : visits.find((v) => v.status === 'completed' && v.assignedPhysio === user.username);
