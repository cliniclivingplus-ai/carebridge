...
// Postgres in production (DATABASE_URL set, e.g. by Render); local JSON-file store otherwise.
...
if (db.isConfigured()) {
  // Sessions survive restarts and work across multiple app instances -- required once this
  // is deployed for real, since express-session's default MemoryStore is single-process only.
...
// In live mode, pull today's appointments once from Clinicea.
...
// one per (feature, operation) pair, e.g. Appointment/Add -> https://<your-host>/webhooks/clinicea/appointment/add/<WEBHOOK_SECRET>
// The trailing secret segment is checked below since Clinicea does not sign its webhook calls.
...
app.post('/webhooks/clinicea/:secret/appointment/add', requireWebhookSecret, handleAppointmentWebhook('add'));
app.post('/webhooks/clinicea/:secret/appointment/edit', requireWebhookSecret, handleAppointmentWebhook('edit'));
app.post('/webhooks/clinicea/:secret/appointment/cancel', requireWebhookSecret, handleAppointmentWebhook('cancel'));
app.post('/webhooks/clinicea/:secret/appointment/delete', requireWebhookSecret, handleAppointmentWebhook('delete'));

// ---------- Demo helpers: simulate what a real Clinicea webhook would send ----------
// Harmless to leave enabled in production (requires login, only writes fake data), but the
// front-end only shows these buttons in mock mode.
...
seedStoreIfEmpty().then(() => {
  app.listen(PORT, () => {
    console.log(`Physio dashboard running at http://localhost:${PORT}`);
